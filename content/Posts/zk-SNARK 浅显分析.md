...
一旦产生余项，就知道 Prover 说谎了。

**Bug**：

可还是有可能，即便说谎，也发生**余项消除**，因为余项的形式是多项式相除（例如 $\dfrac{{7x-6}}{t(x)}$），这种情况下协议失效。

除了余项消除导致协议失效，还可以这样：

- 恶意生成一对满足的 $h(r)$ 和 $p(r)$。Prover 根据 $r$ 计算出 $t(r)$，然后随便找一个随机多项式 $h(x)$，代入 $r$ 得到 $h(r)$，再让 $h(r)$ 和 $t(r)$ 想成，得到 $p(r)$，这样能伪造的同时，不会产生余项。
